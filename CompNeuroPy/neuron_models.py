...
Izhikevich2007_Corbit = Neuron(
    ...
    """,
    equations="""
        C * dv/dt  = k*(v - v_r)*(v - v_t)*(1+pos(v - v_t)**1.5) - u + I_app
        du/dt      = a*(b*(v - v_r) - u)
    """,
    spike = "v >= v_peak",
    reset = """
...
H_and_H_Corbit = Neuron(
  ...
  # Kv3 current with n particle (but in Corbit et al., 2016 paper version h): 
        ...
        dn_Kv3/dt = (n_Kv3_inf - n_Kv3) / tau_n_Kv3_inf : init=0.00021575083389640376
        I_Kv3 = (v - E_Kv3) * gg_Kv3 * pow(n_Kv3,2) #CHECK
    
  # Kv1 current with m, h particle : 
        dm_Kv1/dt = (1.0 / (1.0 + exp((th_m_Kv1 - v) / k_m_Kv1)) - m_Kv1) / tau_m_Kv1 : init=0.27091247265099183
        dh_Kv1/dt = (1.0 / (1.0 + exp((th_h_Kv1 - v) / k_h_Kv1)) - h_Kv1) / tau_h_Kv1 : init=0.4916657721357076
        I_Kv1 = (v - E_Kv1)  * gg_Kv1 * pow(m_Kv1, 3) * h_Kv1 #CHECK
        

        C_m * dv/dt  = -I_L - I_Na - I_Kv3 - I_Kv1 + I_app : init=-69.79996000378554
...
H_and_H_Corbit_voltage_clamp = Neuron(
  ...
  # Kv1 current with m, h particle : 
        ...
        C_m * dv/dt  = 0 : init=-69.79996000378554
...
H_and_H_Corbit_syn = Neuron(
  ...
  # Kv1 current with m, h particle : 
        ...
        C_m * dv/dt  = -I_L - I_Na - I_Kv3 - I_Kv1 + I_app - g_ampa*(v - E_ampa) - g_gaba*(v - E_gaba) : init=-69.79996000378554
