...
class opt_neuron:
    ...
    def __test_variables__(self):
        ...
        ### collect all names
        fitting_vars_names = [
            self.fv_space[i].pos_args[0].pos_args[0]._obj
            for i in range(len(self.fv_space))
        ]
        all_vars_names = np.concatenate(
            [np.array(list(self.const_params.keys())), np.array(fitting_vars_names)]
        ).tolist()
            ...
            if isinstance(val, str):
                try:
                    ...
                    setattr(
                        ...
                        key,
                        fitparams[np.where(np.array(fitting_vars_names) == val)[0][0]],
                    )
                except:
                    try:
    ...
    def __test_fit__(self, fitparamsDict):
        ...
        return self.__run_simulator_with_results__(
            [fitparamsDict[name] for name in fitting_vars_names]
        )
