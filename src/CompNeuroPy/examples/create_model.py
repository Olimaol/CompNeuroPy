...
def two_poisson(params=None, a=0, b=0, c=0):
    ...
    if params is None:
        assert False, 'ERROR: parameter dictionary missing for "two_poisson"'
...
my_model = generate_model(
    ...
    model_kwargs={
        ...
        "b": 1,
        "c": 2,
    },  # if the model_creation_function uses arguments they can be specified in this dictionary
...
)  # if do_create and do_compile would be True it would directly compile using this compile_folder
...
def main():
    ...
    # f"\n{"first model":<45} {"second model":<45}"
    print(
        "{:<45} {:<45}".format(my_model.name, my_2nd_model.name)
    )  # names (if no name was given --> 'model{+number}')
    print(
        "{:<45} {:<45}".format(str(my_model.populations), str(my_2nd_model.populations))
    )  # created populations
    print(
        "{:<45} {:<45}".format(str(my_model.projections), str(my_2nd_model.projections))
    )  # created projections
    print(
        "{:<45} {:<45}".format(my_model.description, my_2nd_model.description)
    )  # descriptions
